...
int main(int argc, char **argv)
{
    ...
    champ(card, "xEq", "4*sin(u)*sin(v)*cos(u)");
    champ(card, "yEq", "4*sin(u)*cos(v)*cos(u)");
    champ(card, "zEq", "4*sin(u)*sin(u)*cos(v)*sin(v)");
    champ(card, "StartU", "0");    champ(card, "EndU", "pi+.1");
    champ(card, "StartV", "0");    champ(card, "EndV", "2*pi+.1");
    champ(card, "IncremU", "pi/24"); champ(card, "IncremV", "pi/24");
